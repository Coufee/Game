...
func NewHeartBeat (kp int, session HeartBeater) *HeartBeat {
	...
	return &HeartBeat{
		...
		to:		time.AfterFunc(time.Second * time.Duration(kp) * TIME_OUT_FACTER, func() {session.TimeOut()}),
	}
}

func (this *HeartBeat) Reset() {
	this.hb.Reset(time.Second * time.Duration(this.keep))
	this.to.Reset(time.Second * time.Duration(this.keep) * TIME_OUT_FACTER)
}
...
func (this *HeartBeatM) Register(kp int, session Sessioner) {
	this.Lock()
	if session != nil {
		if v, exist := this.hbm[session.RemoteAdder()]; exist {
			v.Reset()
		} else {
			this.hbm[session.RemoteAdder()] = NewHeartBeat(kp, session)
		}
	}
	this.Unlock()
}
